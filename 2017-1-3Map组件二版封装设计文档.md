...
方法名称 | 返回值 | 说明
--------- | ------------- | -------------
`on(target,eventName, handler, context)` | `EventListener` | 注册事件，给Map或者覆盖物对象注册事件。target：注册事件对象，eventName：事件名称（必填），handler：事件回调函数（必填），context：事件回调中的上下文（可选，缺省时，handler中this为调用on方法的对象本身，否则this指向context引用的对象）<br><span style="color:red">注意：多次绑定时，当eventName、handler函数对象、context对象有任意一个不一样就会再次绑定。</span>
